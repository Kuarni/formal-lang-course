...
class TestRegexToDfa:
    ...
    def test_simple_regex(self):
        dfa = regex_to_dfa("a")
        assert dfa.is_deterministic()
        assert dfa.accepts(str2symbols("a"))
        assert not dfa.accepts(str2symbols("b"))
        assert not dfa.accepts(str2symbols("ab"))

    def test_concat_symbols(self):
        dfa = regex_to_dfa("a b.o.b a")
        assert dfa.accepts(str2symbols("aboba"))
        assert not dfa.accepts(str2symbols("aboba "))
        assert not dfa.accepts(str2symbols("abob"))

    def test_union(self):
        dfa = regex_to_dfa("a|b+c")
        assert dfa.accepts(str2symbols("a"))
        assert dfa.accepts(str2symbols("b"))
        assert dfa.accepts(str2symbols("c"))
        assert not dfa.accepts(str2symbols("bbc"))

    def test_kleene(self):
        dfa = regex_to_dfa("a*b*")
        assert dfa.accepts(str2symbols(""))
        assert dfa.accepts(str2symbols("aaaa"))
        assert dfa.accepts(str2symbols("bbbb"))
        assert dfa.accepts(str2symbols("aaaabbb"))
        assert not dfa.accepts(str2symbols("ba"))

    def test_groups(self):
        dfa = regex_to_dfa("(b.a)*")
        assert dfa.accepts(str2symbols(""))
        assert dfa.accepts(str2symbols("ba"))
        assert dfa.accepts(str2symbols("baba"))
        assert not dfa.accepts(str2symbols("aaa"))

    def test_epsilon(self):
        dfa = regex_to_dfa("(a|epsilon) (c|$)")
        assert dfa.accepts(str2symbols("ac"))
        assert dfa.accepts(str2symbols("a"))
        assert dfa.accepts(str2symbols("c"))
        assert dfa.accepts(str2symbols(""))
        assert not dfa.accepts(str2symbols("b"))
        assert not dfa.accepts(str2symbols("ac "))
    ...
    def test_big_symbol(self):
        ...
        assert not dfa.accepts(str2symbols("abc"))
