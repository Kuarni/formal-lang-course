...
class MsBfsRpq:
    ...
    __adj_united: Dict[Symbol, sp.spmatrix]
    __init_front: sp.spmatrix
    __front_left: sp.spmatrix

    def __unite_matrices(self) -> dict[Symbol, sp.spmatrix]:
        return {
    ...
    def __get_init_front(self) -> sp.spmatrix:
        ...
        return reduce(lambda a, b: sp.block_array([[a], [b]]), vectors).tocsr()
    ...
    def __update_front(self, front_right: sp.spmatrix) -> sp.lil_matrix:
        def front_mul_matrix(cur_front, matrix) -> sp.lil_matrix:
            mul = sp.csr_matrix(cur_front @ matrix)
            diag_front_right = sp.lil_matrix(front_right.shape, dtype=bool)
        ...
        updated_front = reduce(
            ...
            sp.lil_matrix(front_right.shape, dtype=bool),
        )

        return updated_front

    def __visited_to_dict(self, visited: sp.csr_matrix) -> dict[int, set[int]]:
    ...
    def __ms_bfs(self) -> dict[int, set[int]]:
        front_right = sp.lil_matrix(self.__init_front[:, self.__shift :], dtype=bool)
        visited = sp.csr_matrix(front_right, dtype=bool)
...
def ms_bfs_based_rpq(
    ...
    result = MsBfsRpq(adj_dfa, adj_nfa)()
